...
int mysh_cd(char **argus);
int mysh_help(char **argus);
int mysh_exit(char **argus);
...
int mysh_cd(char **argus)
{
    if(argus[1]==NULL)
    {
        perror("mysh error at cd,lack of argus\n");
...
int  mysh_execute(char** argus)
{
    int i;
    if(argus[0]==NULL)return 1;
    for( i=0;i<mysh_builtin_nums();i++)
        if(strcmp(argus[0],builtin_cmd[i])==0)
            return (*builtin_func[i])(argus);
    return mysh_launch(argus);
}
